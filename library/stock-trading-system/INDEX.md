...
> 本体系由 book2skill v2.0 多书协同蒸馏，基于 13 本交易/投资经典跨书整合，共产出 **13** 个 skills。
> 处理时间: 2026-06-19

## 关于本体系

- **类型**: 多书协同（跨 13 本经典）
- **核心流派**: 趋势交易（执行层）+ 均值回归（震荡市执行层）+ 价值投资（辅助层）+ 东方哲学（心态层）
...
- [`05-breakout-entry`](./05-breakout-entry/SKILL.md) — 突破入场：10/20日突破+涨停过滤+T+1处理+利弗莫尔关键点+ADX<15硬过滤+ATR自适应
...
- [`08-trend-exit`](./08-trend-exit/SKILL.md) — 趋势出场：20日最低价+浮盈>30%回撤15%减仓/浮盈>50%回撤20%清仓+黑天鹅冷静期恢复+快速止损
- [`13-mean-reversion`](./13-mean-reversion/SKILL.md) — 均值回归：RSI<30+布林带下轨+量缩确认+恐慌放量见底例外+bb_mid减仓50%+bb_upper清仓+跌停过滤
...
5. **05-breakout-entry** — 入场执行（趋势市），依赖01+02+03+04
5a. **13-mean-reversion** — 入场执行（震荡市），与05互斥，依赖01+市场状态判断
6. **06-position-sizing** — 头寸计算，依赖05/13入场信号
...
| 出场规则 | 只用20日线 | 20日最低价+浮盈回撤保护 | 08 |
| test-prompts | 4处P0错误 | 全部修正 | 全部 |
| ADX过滤 | 无趋势强度过滤 | ADX<15硬过滤（极端无趋势拒绝入场） | 05 |
| ATR自适应 | 固定用ATR14 | 高波动用atr10/低波动用atr20 | 05,06 |
| 黑天鹅冷静期 | 冷静后无恢复流程 | 冷静期满3日后重新跑01全流程校验 | 08 |
| 快速止损 | 入场后跌破ma20等2日 | 持仓≤3日跌破ma20直接清仓 | 08 |
| 均值回归出场 | bb_mid直接清仓 | bb_mid减仓50%+bb_upper清仓 | 13 |
| 均值回归量能 | vol_ratio<1.0过严 | 放宽至<1.5+恐慌放量见底例外(vol>2.0且RSI<20) | 13 |
| 均值回归跌停 | 无跌停过滤 | 跌停封板拒绝入场 | 13 |
| 回测定价 | 清仓/减仓用entry_price | 改用当日收盘价 | backtest |
| 持仓天数 | hold_days='N/A' | 计算实际持仓天数 | backtest |
| 个股黑天鹅 | 无个股级跌幅检查 | 个股单日跌≥8%减仓至半仓/≥10%清仓+冷静3日 | 01, backtest |
| ATR自适应三档 | 1x阈值无常规档 | 1.5x高波用atr10/0.7x低波用atr20/常规用atr14 | 05,06 |
| 05/13市场门控 | 无策略门控 | trending跳过13，ADX<15跳过所有入场 | 05,13 |
