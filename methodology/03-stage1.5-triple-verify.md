...
不同类型的书,V3 的"常识判定标准"需要调整:

**实操类 / 投资类** (炒股的智慧、段永平):
- 标准: 严格。现代投资常识已经很丰富,只有真正反直觉的才通过
- 例: "止损要快" 是常识,不通过; "止损位应该设在逻辑失效点而非固定百分比" 是独特见解,通过

**教材类** (曼昆经济学):
- 标准: 看是否提供了**决策可用的框架**,而非纯理论复述
- 例: "供需决定价格" 是常识,不通过; "用弹性判断调价对收入的影响方向" 是可操作框架,通过

**哲学类** (易经的智慧) ★:
- 标准: 看**原始表述是否反直觉**,而不是看**当前是否已普及**
- 原因: 易经的很多思想已经融入日常语言 ("亢龙有悔" "物极必反"),但在易经的语境中它们是**系统性的决策框架**,不是零散的成语
- 判定: 如果这个单元在书中是**系统化方法论的一部分** (有完整的判断-行动逻辑),即使概念本身已普及,也通过
- 例: "阴阳转化" 单独看是常识,但易经把它系统化为"阴阳消长的六阶段模型" (六爻),这是独特的决策框架,通过

**政策类** (共同富裕):
- 标准: 看是否提供了**独特的决策约束视角**,而非价值口号
- 例: "要实现共同富裕" 是口号,不通过; "三次分配的边界条件" 是独特政策框架,通过
