...
```
用户说"拆书/蒸馏书/把书做成skill"
  ├─ 书数 = 1 → 单本模式 → 阶段0(Adler) → 阶段1(6并行提取) → 阶段1.5(三重验证) → 阶段2(RIA++) → 阶段3(链接) → 阶段4(压测)
  ├─ 书数 ≥ 2 或说"融合/知识体系/指导决策" → 多书模式 → 步骤1(扫描) → 步骤2(逐本单本流水线) → 步骤3(协同层) → 步骤4(决策产出)
  └─ 输入不完整 (没路径/没书名/模式不明) → 停下问用户,不凭记忆拆书
```
...
**任何 prompt 进入执行流程前,必须先通过此门控。** 缺任何一项都停下来问用户,不要假设或凭记忆补全。

| 必填项 | 检查方式 | 缺失时行为 |
|---|---|---|
| 书的文本来源 (PDF/EPUB/TXT/DOCX 路径或纯文本) | 用户是否提供了可访问的文件路径或文本? | 停下问:"请提供书的文件路径或文本内容。book2skill 不凭记忆拆书。" |
| 书名 + 作者 | 用户是否明确说了书名?作者可后补 | 停下问:"哪本书?请提供书名(作者可选)。" |
| 模式选择 (单本/多书) | 书数 ≥ 2 或用户说"融合/批量/知识体系" → 多书;否则单本 | 模糊时问:"单本还是多书模式?" |
| 决策导向 (多书模式必填) | 多书模式下用户是否说了指导什么决策? | 停下问:"这些书主要想指导什么类型的决策?(商业/投资/管理/个人成长/...)" |
| 首次试点建议 | 用户第一次用 book2skill? | 主动建议:"建议先拆 1 本验证流程,再批量。要试点吗?" |

**通过门控后**,根据模式选择进入:
- 单本模式 → 阶段 0
- 多书模式 → 多书模式流程步骤 1

**极简输入处理** (如用户只说"拆书"): 必须停下询问上述前 3 项,不允许跳过门控直接执行。
